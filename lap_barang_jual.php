...
$pdf->Cell(25.5,0.7,"Laporan Data Barang Penjualan",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$pdf->Cell(4, 0.8, 'Total Harga', 1, 0, 'C');
$pdf->Cell(3, 0.8, 'Laba', 1, 0, 'C');
$pdf->Cell(3, 0.8, 'Diskon', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;

$query= mysqli_query($conn, "SELECT * FROM barang_laku ORDER BY id DESC");
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['tanggal'],1, 0, 'C');
	$pdf->Cell(5, 0.8, $lihat['nama'], 1, 0,'C');
	$pdf->Cell(3, 0.8, $lihat['jumlah'],1, 0, 'C');
	$pdf->Cell(3, 0.8, number_format($lihat['harga']),1, 0, 'C');
	$pdf->Cell(4, 0.8, number_format($lihat['total_harga']),1, 0, 'C');
	$pdf->Cell(3, 0.8, number_format($lihat['laba']),1, 0, 'C');
	$pdf->Cell(3, 0.8, number_format($lihat['diskon']),1, 1, 'C');
	$no++;
}

$pdf->Output("laporan_barang_jual.pdf","I");
